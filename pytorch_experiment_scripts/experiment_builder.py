...
class ExperimentBuilder(nn.Module):
    ...
    def plot_grad_flow(self, named_parameters):
        ...
        for name, params in named_parameters:
            s = name.split('.')
            if all(element in name for element in ['block','weight']):
                all_grads.append(params.grad.abs().mean().cpu())
                layers.append(s[1]+'_'+s[3])
            if all(element in name for element in ['input','weight']):
                all_grads.append(params.grad.abs().mean().cpu())
                layers.append(s[1]+'_'+s[3])
            if all(element in name for element in ['logit_linear','weight']):
                layers.append(s[1]+'_'+s[0])
                all_grads.append(params.grad.abs().mean().cpu())
